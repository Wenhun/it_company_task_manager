...
class PublicAccessTests(TestCase):
    def test_worker_login_required(self) -> None:
        res = self.client.get(WORKER_LIST_URL)
        self.assertNotEqual(res.status_code, 200)

    def test_project_login_required(self) -> None:
        res = self.client.get(PROJECT_LIST_URL)
        self.assertNotEqual(res.status_code, 200)

    def test_task_login_required(self) -> None:
        res = self.client.get(TASK_LIST_URL)
        self.assertNotEqual(res.status_code, 200)

    def test_team_login_required(self) -> None:
...
class PrivateAccessTests(TestCase):
    ...
    def test_retrieve_projects(self) -> None:
        Project.objects.create(
            project_name="test1", deadline="2025-01-01", status="Active"
        )
        Project.objects.create(
            project_name="test2", deadline="2025-01-01", status="Active"
        )
    ...
    def test_retrieve_worker(self) -> None:
        ...
        get_user_model().objects.create_user(
            username="test_1", password=password, position=self.position
        )

        get_user_model().objects.create_user(
            username="test_2", password=password, position=self.position
        )
...
class CreateTest(TestCase):
    def setUp(self) -> None:
        ...
        self.project = Project.objects.create(
            project_name="test", deadline="2025-01-01", status="Active"
        )
    ...
    def test_task_create_from_worker_page_with_worker(self) -> None:
    ...
    def test_task_create_from_project_page_with_project(self) -> None:
        url = reverse("task_manager:task-create")
...
class SetTaskAsCompletedTest(TestCase):
    ...
    def test_setup_task_as_completed(self) -> None:
        response = self.client.post(
